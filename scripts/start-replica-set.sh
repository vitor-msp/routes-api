...
docker exec routes-mongo1 mongosh -u $MONGO_INITDB_ROOT_USERNAME -p $MONGO_INITDB_ROOT_PASSWORD --eval '
    rs.initiate({
        _id: "rs0",
        members: [
            { _id: 0, host: "routes-mongo1:27017", priority: '$MONGO_HOST1_PRIO' },
            { _id: 1, host: "routes-mongo2:27017", priority: '$MONGO_HOST2_PRIO' }
        ]
    })
'
